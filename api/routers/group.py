...
async def group_user_invite(invite_body: group_schema.PublicGroupInvite, db: AsyncSession = Depends(get_db)):
    result = await group_crud.group_user_invite(db, invite_body)
    if result is None:
