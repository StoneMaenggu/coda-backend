...
async def post_create(post_body: post_schema.PostCreate, db: AsyncSession = Depends(get_db)):
    # post table
    post_id = await post_crud.post_create(db, post_body)
    ...
    for i in range(len(post_body.picture_paths)):
        ...
        await post_crud.picture_create(db, picture_data)
    ...
    for i in range(len(post_body.drawing_paths)):
        ...
        await post_crud.drawing_create(db, drawing_data)

    return post_schema.PostCreateResponse(post_id=post_id)

# 15. 개별 포스트 정보 조회
@router.get("/posts/{post_id}", response_model=post_schema.Post)
async def post_contents(post_id: int, db: AsyncSession = Depends(get_db)):
    return await post_crud.post_contents(db, post_id)

# 16. 개별 포스트 수정
@router.patch("/posts/{post_id}/{drawing_id}", response_model=post_schema.CaptionReviseResponse)
async def post_revise(post_id: int, drawing_id: int, post_body: post_schema.CaptionRevise, db: AsyncSession = Depends(get_db)):
    result = await post_crud.post_revise(db, post_id, drawing_id, post_body)
    if result is None:
...
async def post_delete(post_id: int, db: AsyncSession = Depends(get_db)):
    await post_crud.post_delete(db, post_id)
    return